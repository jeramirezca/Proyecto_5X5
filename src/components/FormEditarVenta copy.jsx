...
const obtenerVentas = async (setVentas, setEjecutarConsulta) => {
  const options = { method: 'GET', url: 'https://limitless-wildwood-65072.herokuapp.com/ventas/' };
  await axios
    .request(options)
...
function FormEditarVenta({ venta }) {
  ...
  const submitFormulario = async (e) => {
    ...
    // setInfoNuevoVenta({ ...infoNuevoVenta, productos: listaProductos});
  
    //REALIZA LO DEL BACK PARA QUE LLEVE AL BACK LOS DATOS
    const options = {
      method: 'PATCH',
      url: `https://limitless-wildwood-65072.herokuapp.com/ventas/${venta._id}/`,
      headers: { 'Content-Type': 'application/json' },
      data: { ...infoNuevoVenta, productos: listaProductos },
    };
    ...
    <div className="content-general margen-out">
      <div className="max-width">
        ...
        <form ref={form} onSubmit={submitFormulario} className="table-top">
          ...
          <div className="mt-20 prueba2">
            ...
            <select name="vendedor" value={infoNuevoVenta.vendedor._id} className="form-control" defaultValue={infoNuevoVenta.vendedor} required
              ...
              {vendedores.map((vendedor) => {
                return <option key={nanoid()} value={vendedor._id}> {`${vendedor.usuario}`}</option>;
              })}
          ...
          <TablaProductos productos={productos} productosTabla={productosTabla} setProductos={setProductos} setProductosTabla={setProductosTabla} />
          ...
          <div className="boton-side totalVenta">
            ...
            $ <input name="totalVenta" className="form-control width-m" min="1" type="number" value={infoNuevoVenta.totalVenta}
              onChange={(e) => setInfoNuevoVenta({ ...infoNuevoVenta, totalVenta: e.target.value })} />
          </div>
...
const TablaProductos = ({ productos, productosTabla, setProductos, setProductosTabla }) => {
                ...
                <tr key={nanoid()}>
                  ...
                  <td>
                    <label htmlFor={`cantidad_${index}`}>
                      <input required placeholder={fila.cantidad} className="campoEdicion width-s" name={`cantidad_${index}`} id={`cantidad_${index}`}
                       />
                    </label>
                  </td>
                  ...
                  <td>
                    <label htmlFor={`totalProducto_${index}`}>
                      $<input required placeholder={fila.totalProducto} min="1" type="number" className="campoEdicion width-s" name={`totalProducto_${index}`} 
                      />
                    </label>
                  </td>
                  <td>
